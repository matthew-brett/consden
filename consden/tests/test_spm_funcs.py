...
def test_spm_globals():
    ...
    glob_vals = spm_funcs.get_spm_globals(pjoin(HERE, 'three_vols.nii'))
    # Values as calculated using SPM
    expected_values = np.loadtxt(pjoin(HERE, 'global_signals.txt'))
...
def test_spm_hrf():
    ...
    for dt_str in ('0.1', '1', '2.5'):
        ...
        spm_values = np.loadtxt(pjoin(HERE, spm_fname))
